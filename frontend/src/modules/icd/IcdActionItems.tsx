...
interface IcdActionItemsProps {
  icdItemId: string
}

// Repeatable list of tracked actions against an issue/change/decision (own
// code, owner, due date, status, % complete) — reuses the Risk module's
// Mitigation Actions pattern (see docs/ICD_MODULE_PLAN.md Phase 4).
export function IcdActionItems({ icdItemId }: IcdActionItemsProps) {
  const [actions, setActions] = useState<IcdActionItem[]>([])
  ...
  const load = () => {
    setLoading(true)
    api.get<IcdActionItem[]>('/api/v1/icd-action-items/', { params: { icd_item_id: icdItemId } })
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/v1/icd-action-items/', {
        icd_item_id: icdItemId,
  ...
  const updateAction = async (action: IcdActionItem, patch: Partial<IcdActionItem>) => {
    await api.patch(`/api/v1/icd-action-items/${action.id}`, patch)
    load()
  }
