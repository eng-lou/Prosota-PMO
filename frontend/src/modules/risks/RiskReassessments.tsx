...
import type { RiskReassessment } from './types'

interface RiskReassessmentsProps {
  riskId: string
  refreshKey: number
  onLogged: () => void
}
...
// History of "what changed and why" — distinct from rating_narrative (a single
// current-state explanation). Most entries come from the RiskForm's automatic
// prompt when a trigger field changes; the quick-add button below covers the
// case where a review happened but nothing actually changed. Editable/deletable,
// unlike a strict audit trail — this is a working log, not a compliance record.
export function RiskReassessments({ riskId, refreshKey, onLogged }: RiskReassessmentsProps) {
  const [entries, setEntries] = useState<RiskReassessment[]>([])
  ...
  const load = () => {
    setLoading(true)
    api.get<RiskReassessment[]>('/api/v1/risk-reassessments/', { params: { risk_id: riskId } })
      .then(r => setEntries(r.data))
      .catch(() => setError('Failed to load reassessment history'))
      .finally(() => setLoading(false))
  }

  useEffect(load, [riskId, refreshKey])

  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!note.trim()) return
    try {
      await api.post('/api/v1/risk-reassessments/', { risk_id: riskId, note: note.trim() })
      setNote('')
      setAdding(false)
      load()
      onLogged()
  ...
  const startEdit = (entry: RiskReassessment) => {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/api/v1/risk-reassessments/${editingId}`, { note: editingNote.trim() })
      setEditingId(null)
      load()
    } catch {
  ...
  const handleDelete = async (entry: RiskReassessment) => {
    if (!window.confirm('Delete this reassessment entry? This cannot be undone.')) return
    await api.delete(`/api/v1/risk-reassessments/${entry.id}`)
    load()
  }
